...
# Also someone should update the libs/mqtt_remote_method_calls.py file on the robot too (at some point before adi).
...
def main():
    ...
    canvas.bind("<Button-1>", lambda event: left_mouse_click(event, client))
...
# Left mouse click
# ----------------------------------------------------------------------
def left_mouse_click(event, client):
    ...
    client.send_message("on_circle_draw", [my_color, event.x, event.y])
