...
lsm5_ev3_remote_drive.py module that is running on your EV3 (you have to write that module too, but it's easier).
...
Authors: David Fisher and Adi Sethupathy.
...
def main():
    ...
    stop_button['command'] = lambda: space(mqtt_client)
    root.bind('<space>', lambda event: space(mqtt_client))
...
def forward(mqtt_client, left_speed_entry, right_speed_entry):
    print("forward")
    mqtt_client.send_message('front', [int(left_speed_entry.get()), int(right_speed_entry.get())])
...
def right(mqtt_client, left_speed_entry):
    print("turning right")
